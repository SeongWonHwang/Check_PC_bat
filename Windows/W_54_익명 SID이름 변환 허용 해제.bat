...
:: 내보낸 파일에서 LSAAnonymousNameLookup 설정을 확인
findstr /r /c:"LSAAnonymousNameLookup = 1" C:\result\Windows\export\export_ansi.inf >nul
if %ERRORLEVEL% equ 0 (
    set LSAAnonymousNameLookup=1
    echo 네트워크 액세스: 익명 SID/이름 변환 허용: 사용 >> C:\result\Windows\export\W_54.txt
	echo 네트워크 액세스: 익명 SID/이름 변환 허용: 사용
) else (
    set LSAAnonymousNameLookup=0
    echo 네트워크 액세스: 익명 SID/이름 변환 허용: 사용 안 함 >> C:\result\Windows\export\W_54.txt
